...
class LandingPage:
    ...
    def __init__(self,screen,nextFunc):
        ...
        self.Title = Label(0,100,self.fontTitle,text='LOGIN',textColor=pg.Color(self.redColor))
        self.Title.x = self.screen_w/2 + ((self.screen_w/2) - self.Title.getTextWidth())//2
        ...
        self.Username = Label(self.UsernameX,self.UsernameY  + 5 ,self.fontUsername,text='USERNAME :',textColor=pg.Color(self.grayColor))
        self.username = TextInput(self.UsernameX, self.UsernameY,200,30,self.fontText,border_radius=5,border_thikness=2,activeColor=self.redColor,inactiveColor=self.grayColor)
        ...
        self.Password = Label(self.PasswordX,self.PasswordY  + 5 ,self.fontPassword,text='PASSWORD :',textColor=pg.Color(self.grayColor))
        self.password = PasswordInput(self.PasswordX, self.PasswordY,200,30,self.fontText,border_radius=5,border_thikness=2,activeColor=self.redColor,inactiveColor=self.grayColor)

        self.password.rect.x = self.PasswordX + max(self.Password.getTextWidth(),self.Username.getTextWidth()) + 10
        self.username.rect.x = self.UsernameX + max(self.Password.getTextWidth(),self.Username.getTextWidth()) + 10
